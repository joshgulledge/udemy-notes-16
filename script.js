...
// Promise.race([
//   getJson(`https://restcountries.eu/rest/v2/name/honduras`),
//   timeout(0.01),
// ])
...
const loadNPause = async function () {
  try {
    // Load image 1
    let img = await imageGetter('img/img-1.jpg');
    // let variable because it can change
    console.log('Image 1 loaded');
    await wait(2);
    ...
    // Load image 2
    img = await imageGetter('img/img-2.jpg');
    console.log('Image 2 loaded');
    await wait(2);
    img.style.display = 'none';
  } catch (err) {
...
const loadAll = async function (imgArr) {
  try {
    const imgs = imgArr.map(async img => await imageGetter(img));
